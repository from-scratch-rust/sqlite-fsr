...
fn test_COUNT_sql_command_returns_correct_number_of_rows() {
    ...
    let result = sql::execute(["SELECT", "COUNT(*)", "FROM", "apples"].to_vec(), &mut file).unwrap();
...
fn test_COUNT_sql_command_returns_correct_number_of_rows_2() {
    let mut file = DBFile::open(PathBuf::from("./sample.db")).unwrap();
    let result = sql::execute(["SELECT", "COUNT(*)", "FROM", "oranges"].to_vec(), &mut file).unwrap();
...
fn test_COUNT_sql_command_returns_correct_number_of_rows_3() {
    ...
    let result = sql::execute(["SELECT", "COUNT(*)", "FROM", "superheroes"].to_vec(), &mut file).unwrap();
...
fn test_COUNT_sql_command_returns_correct_number_of_rows_4() {
    ...
    let result = sql::execute(["SELECT", "COUNT(*)", "FROM", "companies"].to_vec(), &mut file).unwrap();
...
fn test_SELECT_sql_command_returns_correct_values() {
    let mut file = DBFile::open(PathBuf::from("./sample.db")).unwrap();
    let result: Vec<String> = sql::execute(["SELECT", "name", "FROM", "apples"].to_vec(), &mut file)
                                .unwrap()
                                .iter()
...
fn test_COUNT_sql_command_returns_error_when_table_not_found() {
    let mut file = DBFile::open(PathBuf::from("./sample.db")).unwrap();
    let result = sql::execute(["SELECT", "COUNT(*)", "FROM", "seafood"].to_vec(), &mut file);
...
fn test_sql_command_returns_error_when_command_not_supported() {
    let mut file = DBFile::open(PathBuf::from("./sample.db")).unwrap();
    let result = sql::execute(["WHOOPTY", "COUNT(*)", "FROM", "seafood"].to_vec(), &mut file);
    assert!(result.is_err());
}